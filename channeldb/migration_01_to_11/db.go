...
func Open(dbPath string, modifiers ...OptionModifier) (*DB, error) {
	...
	bdb, err := kvdb.Open(kvdb.BoltBackendName, path, opts.NoFreelistSync, kvdb.DefaultDBTimeout)
	if err != nil {
		return nil, err
	}
...
func createChannelDB(dbPath string) error {
	...
	path := filepath.Join(dbPath, dbName)
	bdb, err := kvdb.Create(kvdb.BoltBackendName, path, false, kvdb.DefaultDBTimeout)
	if err != nil {
		return err
	}
