...
func GetBoltBackend(cfg *BoltBackendConfig) (Backend, error) {
	...
	if !fileExists(dbFilePath) {
		...
		db, err = Create(BoltBackendName, dbFilePath, cfg.NoFreelistSync, DefaultDBTimeout)
	} else {
		db, err = Open(BoltBackendName, dbFilePath, cfg.NoFreelistSync, DefaultDBTimeout)
	}
