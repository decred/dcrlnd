...
func (m *memoryMailBox) SetFeeRate(feeRate chainfee.AtomPerKByte) {
	m.pktCond.L.Lock()
	defer m.pktCond.L.Unlock()
...
func (m *memoryMailBox) DustPackets() (lnwire.MilliAtom,
	lnwire.MilliAtom) {
	...
	var (
		localDustSum  lnwire.MilliAtom
		remoteDustSum lnwire.MilliAtom
	)
	...
	for _, e := range m.addIndex {
		...
		if m.isDust(
			m.feeRate, false, true, addPkt.amount.ToAtoms(),
		) {
		...
		if m.isDust(
			m.feeRate, false, false, addPkt.amount.ToAtoms(),
		) {
