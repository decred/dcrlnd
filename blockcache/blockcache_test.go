...
func TestBlockCacheGetBlock(t *testing.T) {
	...
	inv1 := wire.NewInvVect(wire.InvTypeBlock, &blockhash1)
	inv2 := wire.NewInvVect(wire.InvTypeBlock, &blockhash2)
	inv3 := wire.NewInvVect(wire.InvTypeBlock, &blockhash3)

	// Determine the size of one of the blocks.
	sz, _ := (&cache.CacheableBlock{Block: dcrutil.NewBlock(block1)}).Size()
