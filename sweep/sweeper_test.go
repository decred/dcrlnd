...
func TestZipClusters(t *testing.T) {
	...
	createCluster := func(inp pendingInputs, f chainfee.AtomPerKByte) inputCluster {
		return inputCluster{
			sweepFeeRate: f,
			inputs:       inp,
		}
...
func (i *testInput) CraftInputScript(_ input.Signer, txn *wire.MsgTx,
	...
	//
	// Note(decred) OP_1 + <int> is used instead of int directly because
	// SigScriptToWitnessStack converts OP_0 to a nil byte slice, making it
	// harder to verify. Thus we add OP_1 to ensure a data push actually
	// happens.
	op := i.OutPoint()
	return &input.Script{
		Witness: [][]byte{
			...
			// ..the outpoint index...
			//{byte(op.Index)},
			{txscript.OP_1 + byte(op.Index)},
			// ..and finally the tx input index.
			{txscript.OP_1 + byte(txinIdx)},
		},
	}, nil
}
...
func assertSignedIndex(t *testing.T, tx *wire.MsgTx,
	...
	for idx, txIn := range tx.TxIn {
		...
		require.NoError(t, err)
		require.Equal(t, witness[0], op.Hash[:])
		require.Equal(t, witness[1], []byte{txscript.OP_1 + byte(op.Index)})
		require.Equal(t, witness[2], []byte{txscript.OP_1 + byte(idx)})
		found[op] = struct{}{}
	}
...
func TestRequiredTxOuts(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		input := createTestInput(
			int64(dcrutil.AtomsPerCoin+i*500),
			input.CommitmentTimeLock,
		)
