...
func lockInputs(w lnwallet.WalletController, packet *psbt.Packet) ([]*lnwallet.LockedOutput,
	error) {

	locks := make([]*lnwallet.LockedOutput, len(packet.UnsignedTx.TxIn))
	for idx, rawInput := range packet.UnsignedTx.TxIn {
		lock := &lnwallet.LockedOutput{
