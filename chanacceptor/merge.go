...
// mergeMilliatoms merges two msat values, failing if they have different
// non-zero values.
func mergeMilliatoms(name string, current,
...
func mergeResponse(current, new ChannelAcceptResponse) (ChannelAcceptResponse,
	...
	current.Reserve = dcrutil.Amount(reserve)

	current.MinHtlcIn, err = mergeMilliatoms(
	...
	current.InFlightTotal, err = mergeMilliatoms(
